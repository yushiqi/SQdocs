...
MongoDB 是一个基于分布式文件存储的数据库。MongoDB
...
-  支持
   **Golang**\ ，\ **RUBY**\ ，\ **PYTHON**\ ，\ **JAVA**\ ，\ **C++**\ ，\ **PHP**\ ，\ **C#**
   等多种语言
-  MongoDB安装简单。
...
MongoDB
的主要目标是在键/值存储方式（提供了高性能和高度伸缩性）和传统的RDBMS
系统（具有丰富的功能）之间架起一座桥梁，它集两者的优势于一身。根据官方网站的描述，Mongo
适用于以下场景。
...
-  高伸缩性的场景：Mongo 非常适合由数十或数百台服务器组成的数据库，Mongo
   的路线图中已经包含对MapReduce 引擎的内置支持。
-  用于对象及JSON 数据的存储：Mongo 的BSON
   数据格式非常适合文档化格式的存储及查询。
...
-  ``save()``\ ：如果 \_id
   主键存在则更新数据，如果不存在就插入数据。该方法新版本中已废弃，可以使用
...
-  **update** :
   update的对象和一些更新的操作符（如\ :math:`,`\ inc…）等，也可以理解为sql
   update查询内set后面的
...
pretty() 方法以格式化的方式来显示所有文档。除了 find()
方法之外，还有一个 findOne() 方法，它只返回一个文档。
...
+======+================+==============================+==============+
| 等于 | ``{<key>:<valu | ``db.col.find({"name":"lisi" | ``where name |
|      | e>``}          | }).pretty()``                |  = 'lisi'``  |
+------+----------------+------------------------------+--------------+
| 小于 | ``{<key>:{$lt: | ``db.col.find({"likes":{$lt: | ``where like |
|      | <value>}}``    | 50}}).pretty()``             | s < 50``     |
+------+----------------+------------------------------+--------------+
| 小于或等 | ``{<key>:{$lte | ``db.col.find({"likes":{$lte | ``where like |
| 于   | :<value>}}``   | :50}}).pretty()``            | s <= 50``    |
+------+----------------+------------------------------+--------------+
| 大于 | ``{<key>:{$gt: | ``db.col.find({"likes":{$gt: | ``where like |
|      | <value>}}``    | 50}}).pretty()``             | s > 50``     |
+------+----------------+------------------------------+--------------+
| 大于或等 | ``{<key>:{$gte | ``db.col.find({"likes":{$gte | ``where like |
| 于   | :<value>}}``   | :50}}).pretty()``            | s >= 50``    |
+------+----------------+------------------------------+--------------+
| 不等于 | ``{<key>:{$ne: | ``db.col.find({"likes":{$ne: | ``where like |
|      | <value>}}``    | 50}}).pretty()``             | s != 50``    |
+------+----------------+------------------------------+--------------+
...
MongoDB 使用 ``$regex``
操作符来设置匹配字符串的正则表达式。MongoDB使用PCRE (Perl Compatible
Regular Expression)
作为正则表达式语言。不同于全文检索，我们使用正则表达式不需要做任何配置。
...
这在标签的实现上非常有用，如果需要查找包含以\ **新闻**\ 开头的标签数据，使用以下代码：
...
-  如果正则表达式是前缀表达式，所有匹配的数据将以指定的前缀字符串为开始。例如：
   如果正则表达式为 **^tut** ，查询语句将查找以 tut 为开头的字符串。
...
``sort()``\ 方法可以通过参数指定排序的字段，并使用 1 和 -1
来指定排序的方式，其中 1 为升序排列，而 -1 是用于降序排列。
...
查询一个随机的username，需要知道的是，可以使用\ `explain() <https://docs.mongodb.com/manual/reference/method/cursor.explain/#cursor.explain>`__\ 返回查询过程的详细描述
...
MongoDB中聚合(aggregate)主要用于处理数据(诸如统计平均值,求和等)，并返回计算后的数据结果。有点类似sql语句中的
count(*)。
...
以上实例类似sql语句： **select by_user, count(*) from mycol group by
by_user**
...
:math:`match用于获取分数大于70小于或等于90记录，然后将符合条件的记录送到下一阶段`\ group管道操作符进行处理。
...
-  jsMode说明:为false时
   BSON–>JS–>map–>BSON–>JS–>reduce–>BSON,可处理非常大的mapreduce,为true时
   BSON–>js–>map–>reduce–>BSON
-  Verbose：显示详细的时间统计信息
...
操作将结果输出到名为
``map_reduce_example``\ 的集合。如果\ ``map_reduce_example``\ 集合已经存在，则该操作将用此map-reduce操作的结果替换。
...
此操作使用\ **query**\ 字段选择仅\ **ord_date**\ 大于等于\ **new
Date(“2020-03-01”)**\ 的文档。然后将结果输出到\ ``map_reduce_example2``\ 集合。
...
主从复制是 MongoDB 最早使用的复制方式，
该复制方式易于配置，并且可以支持任意数量的从节点服务器，与使用单节点模式相比有如下优点：
...
主节点的操作记录称为oplog(operation log)，它被存储在 MongoDB 的 local
数据库中。oplog
中的每个文档都代表主节点上执行的一个操作。需要重点强调的是\ **oplog只记录改变数据库状态的操作**\ 。比如，查询操作就不会被存储在oplog中。这是因为oplog只是作为从节点与主节点保持数据同步的机制。
...
MongoDB副本集（Replica
Set）其实就是具有自动故障恢复功能的主从集群，和主从复制最大的区别就是在副本集中没有固定的\ **主节点**\ ；整个副本集会选出一个节点作为\ **主节点**\ ，当其挂掉后，再在剩下的从节点中选举一个节点成为新的\ **主节点**\ ，在副本集中总有一个主节点(primary)和一个或多个备份节点(secondary)。
...
-  **主节点（Primary）**
   接收所有的写请求，然后把修改同步到所有Secondary。一个Replica
   Set只能有一个Primary节点，当Primary挂掉后，其他Secondary或者Arbiter节点会重新选举出来一个主节点。
...
其实副本集（Replica
Set）是主从复制的高级形式。主从复制实现了数据备份+读扩展，但是master一旦down掉，需要手动启动slave。副本集在此基础上实现了备份自动重启的功能，也就是某一台slave会挺身而出，担当起master的职责。所以有三个角色:\ **Primary**\ ，\ **Secondary**\ ，\ **Arbiter**\ 。
...
5. 使用replica set
   tags创建定制的写规则以确保应用能够控制写操作成功的门限值。使用写规则确保操作在返回成功之前将操作传递给指定的数据中心或不同功能的机器。
...
.. figure:: https://www.runoob.com/wp-content/uploads/2013/12/sharding.png
   :alt: img
...
-  Shard:
   ...
   用于存储实际的数据块，实际生产环境中一个shard
   server角色可由几台机器组个一个replica set承担，防止主机单点故障
...
Mongos本身并不持久化数据，Sharded cluster所有的元数据都会存储到Config
Server，而用户的数据会分散存储到各个Shard。Mongos启动后，会从配置服务器加载元数据，开始提供服务，将用户的请求正确路由到对应的碎片。
...
1. mongos对集群进行抽象，让集群“不可见”
   ...
   对于一个读写操作，mongos
   需要知道应该将其路由到哪个复制集上，mongos通过将片键空间划分为若干个区间，计算出一个操作的片键的所属区间对应的复制集来实现路由。
...
在启用访问控制之前，应该创建一个用户，该用户可以在启用访问控制后创建用户并为用户分配角色。然后，这个用户管理员将用于创建和维护其他用户和角色，因此需要分配一个合适的角色（具有
``userAdmin``\ 或\ ``userAdminAnyDatabase``\ 角色）来支持。如果你不创建此管理用户，则在启用访问控制时将无法登录或创建新用户和角色。
...
| Name                              | Description                       |
+===================================+===================================+
| `db.auth() <https://docs.mongodb. | Authenticates a user to a         |
| com/manual/reference/method/db.au | database.                         |
| th/#db.auth>`__                   |                                   |
+-----------------------------------+-----------------------------------+
| `db.changeUserPassword() <https:/ | Changes an existing user’s        |
| /docs.mongodb.com/manual/referenc | password.                         |
| e/method/db.changeUserPassword/#d |                                   |
| b.changeUserPassword>`__          |                                   |
+-----------------------------------+-----------------------------------+
| `db.createUser() <https://docs.mo | Creates a new user.               |
| ngodb.com/manual/reference/method |                                   |
| /db.createUser/#db.createUser>`__ |                                   |
+-----------------------------------+-----------------------------------+
| `db.dropUser() <https://docs.mong | Removes a single user.            |
| odb.com/manual/reference/method/d |                                   |
| b.dropUser/#db.dropUser>`__       |                                   |
+-----------------------------------+-----------------------------------+
| `db.dropAllUsers() <https://docs. | Deletes all users associated with |
| mongodb.com/manual/reference/meth | a database.                       |
| od/db.dropAllUsers/#db.dropAllUse |                                   |
| rs>`__                            |                                   |
+-----------------------------------+-----------------------------------+
| `db.getUser() <https://docs.mongo | Returns information about the     |
| db.com/manual/reference/method/db | specified user.                   |
| .getUser/#db.getUser>`__          |                                   |
+-----------------------------------+-----------------------------------+
| `db.getUsers() <https://docs.mong | Returns information about all     |
| odb.com/manual/reference/method/d | users associated with a database. |
| b.getUsers/#db.getUsers>`__       |                                   |
+-----------------------------------+-----------------------------------+
| `db.grantRolesToUser() <https://d | Grants a role and its privileges  |
| ocs.mongodb.com/manual/reference/ | to a user.                        |
| method/db.grantRolesToUser/#db.gr |                                   |
| antRolesToUser>`__                |                                   |
+-----------------------------------+-----------------------------------+
...
| Name                              | Description                       |
+===================================+===================================+
| `db.createRole() <https://docs.mo | Creates a role and specifies its  |
| ngodb.com/manual/reference/method | privileges.                       |
| /db.createRole/#db.createRole>`__ |                                   |
+-----------------------------------+-----------------------------------+
| `db.dropRole() <https://docs.mong | Deletes a user-defined role.      |
| odb.com/manual/reference/method/d |                                   |
| b.dropRole/#db.dropRole>`__       |                                   |
+-----------------------------------+-----------------------------------+
| `db.dropAllRoles() <https://docs. | Deletes all user-defined roles    |
| mongodb.com/manual/reference/meth | associated with a database.       |
| od/db.dropAllRoles/#db.dropAllRol |                                   |
| es>`__                            |                                   |
+-----------------------------------+-----------------------------------+
| `db.getRole() <https://docs.mongo | Returns information for the       |
| db.com/manual/reference/method/db | specified role.                   |
| .getRole/#db.getRole>`__          |                                   |
+-----------------------------------+-----------------------------------+
| `db.getRoles() <https://docs.mong | Returns information for all the   |
| odb.com/manual/reference/method/d | user-defined roles in a database. |
| b.getRoles/#db.getRoles>`__       |                                   |
+-----------------------------------+-----------------------------------+
| `db.grantPrivilegesToRole() <http | Assigns privileges to a           |
| s://docs.mongodb.com/manual/refer | user-defined role.                |
| ence/method/db.grantPrivilegesToR |                                   |
| ole/#db.grantPrivilegesToRole>`__ |                                   |
+-----------------------------------+-----------------------------------+
| `db.revokePrivilegesFromRole() <h | Removes the specified privileges  |
| ttps://docs.mongodb.com/manual/re | from a user-defined role.         |
| ference/method/db.revokePrivilege |                                   |
| sFromRole/#db.revokePrivilegesFro |                                   |
| mRole>`__                         |                                   |
+-----------------------------------+-----------------------------------+
| `db.grantRolesToRole() <https://d | Specifies roles from which a      |
| ocs.mongodb.com/manual/reference/ | user-defined role inherits        |
| method/db.grantRolesToRole/#db.gr | privileges.                       |
| antRolesToRole>`__                |                                   |
+-----------------------------------+-----------------------------------+
| `db.revokeRolesFromRole() <https: | Removes inherited roles from a    |
| //docs.mongodb.com/manual/referen | role.                             |
| ce/method/db.revokeRolesFromRole/ |                                   |
| #db.revokeRolesFromRole>`__       |                                   |
+-----------------------------------+-----------------------------------+
| `db.updateRole() <https://docs.mo | Updates a user-defined role.      |
| ngodb.com/manual/reference/method |                                   |
| /db.updateRole/#db.updateRole>`__ |                                   |
+-----------------------------------+-----------------------------------+
...
| 语法                     | 描述           | 实例                     |
+==========================+================+==========================+
| mongodump –host          | 该命令将备份所有MongoD | mongodump –host  |
| HOST_NAME –port          | B数据          | runoob.com –port 27017   |
| PORT_NUMBER              |                |                          |
+--------------------------+----------------+--------------------------+
...
+--------------------------+----------------+--------------------------+
| mongodump –collection    | 该命令将备份指定数据库的集合 | mongodump –collection |
| COLLECTION –db DB_NAME   | 。             | mycol –db test           |
+--------------------------+----------------+--------------------------+
...
-  ``<path>``\ ：
   ...
   mongorestore
   最后的一个参数，设置备份数据所在位置，例如：\ ``c:\data\dump\test``\ 。
